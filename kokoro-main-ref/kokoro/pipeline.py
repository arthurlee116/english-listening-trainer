...
class KPipeline:
    ...
    def waterfall_last(
        ...
        for w in waterfall:
            ...
            if z < len(tokens) and tokens[z].phonemes in bumps:
                z += 1
            current_chunk_size = next_count - len(KPipeline.tokens_to_ps(tokens[:z]))
            if current_chunk_size <= target_limit:
                # 如果当前chunk太小，尝试包含更多内容
                if current_chunk_size < target_limit * 0.8:  # 小于80%利用率
                    # 尝试找到更好的分割点
                    remaining_space = target_limit - current_chunk_size
                    for i in range(z, min(z + 20, len(tokens))):  # 向前看20个token
                        test_chunk_size = next_count - len(KPipeline.tokens_to_ps(tokens[:i]))
                        if test_chunk_size <= target_limit:
                            z = i
                        else:
                            break
                return z
        
        # 如果没有找到好的分割点，尽可能填满chunk
        for i in range(len(tokens), 0, -1):
            current_chunk_size = next_count - len(KPipeline.tokens_to_ps(tokens[:i]))
            if current_chunk_size <= target_limit:
                return i
    ...
    def generate_from_tokens(
        ...
        if isinstance(tokens, str):
            ...
            if len(tokens) > 2048:
                raise ValueError(f'Phoneme string too long: {len(tokens)} > 2048')
        ...
        for gs, ps, tks in self.en_tokenize(tokens):
            if not ps:
                ...
                logger.warning(f"Unexpected len(ps) == {len(ps)} > 2048 and ps == '{ps}'")
                logger.warning("Truncating to 2048 characters")
                ps = ps[:2048]
    ...
    def __call__(
        ...
        for graphemes_index, graphemes in enumerate(text):
            ...
            if self.lang_code in 'ab':
                ...
                phonemes, tokens = self.g2p(graphemes)
                for gs, ps, tks in self.en_tokenize(tokens):
                    if not ps:
                        continue
                    elif len(ps) > 2048:
            ...
            # Non-English processing with chunking
                ...
                for chunk in chunks:
                    ...
                    if not ps:
                        ...
                        logger.warning(f'Truncating len(ps) == {len(ps)} > 2048')
                        ps = ps[:2048]
