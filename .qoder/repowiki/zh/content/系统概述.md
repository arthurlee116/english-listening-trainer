...
- [lib/ai/cerebras-client-manager.ts](file://lib/ai/cerebras-client-manager.ts) - *更新于提交 1879a42b*
- [lib/ai/cerebras-service.ts](file://lib/ai/cerebras-service.ts) - *更新于提交 0a55a28e*
</cite>

## 更新摘要
**已更新内容**  
- 根据提交 `1879a42b` 更新了AI服务代理配置说明，移除了动态代理支持的相关描述
- 根据提交 `0a55a28e` 更新了AI服务调用机制，强调统一结构化调用管道
- 修正了AI服务架构图和类图，反映硬编码代理和结构化调用的最新实现
...
AI服务配置由`AIServiceConfig`接口定义，包含API密钥、基础URL、超时设置等参数。系统通过`lib/ai-service.ts`封装了与Cerebras API的交互逻辑，实现了请求重试、超时控制和错误处理等健壮性机制。根据提交 `1879a42b` 的变更，系统已移除动态代理支持，改为硬编码使用生产代理地址 `http://81.71.93.183:10811`，简化了代理配置并提高了连接稳定性。
...
- [lib/ai/cerebras-client-manager.ts](file://lib/ai/cerebras-client-manager.ts#L1-L85) - *更新于提交 1879a42b*
- [lib/ai/cerebras-service.ts](file://lib/ai/cerebras-service.ts#L1-L61) - *更新于提交 0a55a28e*
