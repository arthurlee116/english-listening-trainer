...
AI服务配置由`AIServiceConfig`接口定义，包含API密钥、基础URL、超时设置等参数。系统通过`lib/ai-service.ts`封装了与Cerebras API的交互逻辑，实现了请求重试、超时控制和错误处理等健壮性机制。
