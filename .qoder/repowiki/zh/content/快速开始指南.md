...
1. **首次初始化**：
   ...
   ```
   该脚本会执行以下操作：
   - 在 `kokoro-local/` 目录下创建Python虚拟环境。
...
- 如果问题持续，尝试清除缓存并重新安装：
  ```bash
  rm -rf node_modules package-lock.json kokoro-local/venv
  npm install
  npm run setup-kokoro
  ```
...
2. 手动检查并重建虚拟环境：
   ```bash
   cd kokoro-local && rm -rf venv && python3.12 -m venv venv
   npm run setup-kokoro
   ```
3. 确认语音文件存在于 `kokoro-local/voices/` 目录下。
