...
- [CACHE_OPTIMIZATION_GUIDE.md](file://documents/CACHE_OPTIMIZATION_GUIDE.md) - *新增缓存优化指南*
- [DOCKER_OPTIMIZATION_GUIDE.md](file://documents/DOCKER_OPTIMIZATION_GUIDE.md) - *Docker优化指南*
- [backup.sh](file://scripts/backup.sh) - *增强备份脚本，包含权限修复和压缩选项*
- [Dockerfile.optimized](file://Dockerfile.optimized) - *优化的Docker构建，用于缓存预热和开发环境*
- [DEPLOYMENT.md](file://documents/DEPLOYMENT.md) - *远程服务器部署指南，包含缓存预热和验证流程*
</cite>

## 更新摘要
**变更内容**   
- 更新了Docker多阶段构建策略，优化了依赖管理和缓存机制
- 增强了CI/CD工作流，添加了自动部署钩子和SpaCy模型预装
- 新增了缓存优化文档和故障排除指南
- 更新了部署脚本以支持2025年最佳实践
- 优化了Dockerfile构建过程，提升了性能
- 增强了备份脚本，增加了权限修复和压缩选项
- 新增了远程缓存预热和验证机制
- 统一了Dockerfile中的Node.js版本

**新增章节**
- [Docker缓存优化策略](#docker缓存优化策略)
- [CI/CD流程增强](#cicd流程增强)
- [远程缓存预热与验证](#远程缓存预热与验证)
...
5. [GPU加速支持配置](#gpu加速支持配置)
6. [反向代理与Nginx集成](#反向代理与nginx集成)
7. [自动化部署脚本详解](#自动化部署脚本详解)
8. [CI/CD流程增强](#cicd流程增强)
9. [服务健康监测机制](#服务健康监测机制)
10. [管理员服务器独立运行模式](#管理员服务器独立运行模式)
11. [容量规划建议](#容量规划建议)
12. [日志收集策略](#日志收集策略)
13. [备份恢复流程](#备份恢复流程)
14. [监控告警设置](#监控告警设置)
...
- [DEPLOYMENT.md](file://documents/DEPLOYMENT.md#L1-L603) - *远程服务器部署指南*
...
最终生成的运行时镜像仅包含必要组件，显著减小体积并提升安全性。通过`--chown=nextjs:nodejs`确保文件权限正确，使用`dumb-init`作为PID 1进程处理信号转发。构建过程采用缓存挂载优化，加速CI/CD流程。
...
**Diagram sources**
- [Dockerfile](file://Dockerfile#L1-L195) - *重构的多阶段构建*
- [Dockerfile.optimized](file://Dockerfile.optimized#L1-L133) - *优化的Docker构建*

**Section sources**
- [Dockerfile](file://Dockerfile#L1-L195) - *优化的Docker构建过程*
- [Dockerfile.optimized](file://Dockerfile.optimized#L1-L133) - *优化的Docker构建，用于缓存预热*
- [DOCKER_OPTIMIZATION_GUIDE.md](file://documents/DOCKER_OPTIMIZATION_GUIDE.md) - *Docker优化指南*
...
I & J & K --> N[缓存命中]
L & M --> O[增量构建]
P[CACHE_OPTIMIZATION_GUIDE.md] --> Q[最佳实践]
P --> R[故障排除]
N & O & Q --> S[快速CI/CD流水线]
```
...
- [Dockerfile](file://Dockerfile#L1-L195) - *缓存挂载配置*
- [CACHE_OPTIMIZATION_GUIDE.md](file://documents/CACHE_OPTIMIZATION_GUIDE.md) - *缓存优化文档*

**Section sources**
- [.github/workflows/build-and-push.yml](file://.github/workflows/build-and-push.yml#L1-L150) - *CI/CD工作流优化*
- [Dockerfile](file://Dockerfile#L1-L195) - *Docker缓存配置*
- [CACHE_OPTIMIZATION_GUIDE.md](file://documents/CACHE_OPTIMIZATION_GUIDE.md) - *缓存优化指南*
- [CACHE_TROUBLESHOOTING.md](file://documents/CACHE_TROUBLESHOOTING.md) - *缓存故障排除*

## 远程缓存预热与验证

为解决远程服务器部署时缓存缺失问题，系统实现了远程缓存预热与验证机制。通过预先拉取多级缓存层，将部署时的下载量从3-4GB减少到<300MB，部署时间减少90%以上。

`DEPLOYMENT.md`文档提供了完整的远程服务器部署指南，包含缓存预热和验证流程。虽然专用的`remote-cache-prewarm.sh`和`verify-cache-layers.sh`脚本已归档，但其核心逻辑已整合到手动部署流程中。

部署前，通过`docker pull`命令依次拉取`cache-base`、`cache-python`和`cache-node`缓存层。使用`docker images`命令验证缓存层的完整性和大小。此流程确保了部署时只需下载业务代码层，极大提升了部署效率。
...
- [DEPLOYMENT.md](file://documents/DEPLOYMENT.md#L1-L603) - *远程服务器部署指南*
- [Dockerfile.optimized](file://Dockerfile.optimized#L1-L133) - *优化的Docker构建，用于缓存预热*
- [workflow-snapshots/remote-cache-prewarm-snapshot.md](file://documents/workflow-snapshots/remote-cache-prewarm-snapshot.md#L1-L237) - *远程缓存预热工作流快照*
...
- [Dockerfile](file://Dockerfile#L1-L195) - *PyTorch GPU安装*

**Section sources**
- [docker-compose.gpu.yml](file://docker-compose.gpu.yml#L1-L127) - *GPU资源配置*
- [Dockerfile](file://Dockerfile#L1-L195) - *GPU依赖安装*
...
- [DOCKER_OPTIMIZATION_GUIDE.md](file://documents/DOCKER_OPTIMIZATION_GUIDE.md) - *部署最佳实践*
...
**Diagram sources**
- [.github/workflows/build-and-push.yml](file://.github/workflows/build-and-push.yml#L1-L150) - *CI/CD工作流*
- [Dockerfile](file://Dockerfile#L1-L195) - *预装SpaCy模型*
- [.kiro/hooks/auto-deploy-on-push.md](file://.kiro/hooks/auto-deploy-on-push.md) - *自动部署钩子*

**Section sources**
- [.github/workflows/build-and-push.yml](file://.github/workflows/build-and-push.yml#L1-L150) - *CI/CD工作流*
- [Dockerfile](file://Dockerfile#L1-L195) - *构建优化*
...
`backup.sh`脚本已增强，包含权限修复和压缩选项。脚本会自动检查备份目录权限，若权限不足则尝试使用sudo修复。支持`--compress`和`--no-compress`选项控制备份文件格式，并可通过`--cleanup`选项清理旧备份。

**Section sources**
- [restore.sh](file://scripts/restore.sh#L1-L552) - *数据恢复脚本*
- [scripts/backup.sh](file://scripts/backup.sh#L1-L499) - *增强的备份脚本*
- [scripts/backup.sh](file://scripts/backup.sh#L1-L499) - *权限修复和压缩选项*
